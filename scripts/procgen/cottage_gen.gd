...
const HALF_X: float = 2.5
const HALF_Z: float = 2.0
...
static func build(seed_value: int) -> Dictionary:
	...
	var roof_tilt: float = deg_to_rad(38.0 + rng.randf_range(-6.0, 6.0))
	...
	for beam_z: float in [-HALF_Z, HALF_Z]:
		var beam: MeshInstance3D = MeshLib.mesh_instance(
			MeshLib.plank(Vector3(HALF_X * 2.0 - 0.3, 0.24, 0.3)), wood, "BaseBeamX"
		)
		beam.position = Vector3(0.0, 0.42, beam_z)
		_add_piece(root, result, "foundation", beam)
	for beam_x: float in [-HALF_X, HALF_X]:
		var beam: MeshInstance3D = MeshLib.mesh_instance(
			MeshLib.plank(Vector3(0.3, 0.24, HALF_Z * 2.0 - 0.3)), wood, "BaseBeamZ"
		)
	...
	for beam_z: float in [-HALF_Z, HALF_Z]:
		var beam: MeshInstance3D = MeshLib.mesh_instance(
			MeshLib.plank(Vector3(HALF_X * 2.0 + 0.2, 0.2, 0.24)), wood, "TopBeamX"
		)
		beam.position = Vector3(0.0, WALL_H + 0.1, beam_z)
		_add_piece(root, result, "frame", beam)
	for beam_x: float in [-HALF_X, HALF_X]:
		var beam: MeshInstance3D = MeshLib.mesh_instance(
			MeshLib.plank(Vector3(0.24, 0.2, HALF_Z * 2.0 + 0.2)), wood, "TopBeamZ"
		)
		beam.position = Vector3(beam_x, WALL_H + 0.1, 0.0)
		_add_piece(root, result, "frame", beam)
	var ridge_h: float = WALL_H + tan(roof_tilt) * HALF_Z
	for gable_x: float in [-HALF_X, HALF_X]:
	...
	for row: int in plank_rows:
		...
		for wall_z: float in [-HALF_Z, HALF_Z]:
			var plank: MeshInstance3D = MeshLib.mesh_instance(
				MeshLib.plank(Vector3(HALF_X * 2.0 - 0.3, row_h - 0.04, 0.14)), wood, "WallX"
			)
			plank.position = Vector3(0.0, y, wall_z)
			_add_piece(root, result, "walls", plank)
		var back: MeshInstance3D = MeshLib.mesh_instance(
			MeshLib.plank(Vector3(0.14, row_h - 0.04, HALF_Z * 2.0 - 0.3)), wood, "WallBack"
		)
		back.position = Vector3(-HALF_X, y, 0.0)
	...
	for door_side: float in [-1.0, 1.0]:
		var strip: MeshInstance3D = MeshLib.mesh_instance(
			MeshLib.plank(Vector3(0.14, WALL_H - 0.55, HALF_Z - 0.65)), wood, "WallFront"
		)
		strip.position = Vector3(
			HALF_X, 0.55 + (WALL_H - 0.55) * 0.5, door_side * (HALF_Z * 0.5 + 0.32)
		)
		_add_piece(root, result, "walls", strip)

	# --- Tejado y acabado ---
	var ridge: MeshInstance3D = MeshLib.mesh_instance(
		MeshLib.plank(Vector3(HALF_X * 2.0 + 0.5, 0.18, 0.18)), wood_alt, "Ridge"
	)
	ridge.position = Vector3(0.0, ridge_h, 0.0)
	_add_piece(root, result, "roof", ridge)
	var panel_len: float = sqrt(pow(HALF_Z + 0.35, 2.0) + pow(ridge_h - WALL_H, 2.0)) + 0.2
	for roof_side: float in [-1.0, 1.0]:
		var panel: MeshInstance3D = MeshLib.mesh_instance(
			MeshLib.plank(Vector3(HALF_X * 2.0 + 0.6, 0.12, panel_len)), palette.roof, "RoofPanel"
		)
		panel.rotation.x = roof_side * roof_tilt
		var mid_z: float = roof_side * (HALF_Z + 0.35) * 0.5
	...
	door.position = Vector3(HALF_X + 0.02, 0.55 + 0.9, 0.0)
	_add_piece(root, result, "roof", door)
	var window_positions: Array[Vector3] = [
		Vector3(0.0, 1.55, -HALF_Z - 0.02),
		Vector3(0.0, 1.55, HALF_Z + 0.02),
		Vector3(-HALF_X - 0.02, 1.55, 0.0),
	]
	...
	if has_bench:
		...
		bench.position = Vector3(HALF_X + 0.65, 0.22, HALF_Z * 0.62)
