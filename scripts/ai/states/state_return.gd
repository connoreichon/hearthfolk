...
func tick(dt: float) -> void:
	...
	# Objetivo inalcanzable (isla residual): no perseguir imposibles
	var unreachable: bool = _timeout < 23.0 and not citizen.nav_agent.is_target_reachable()
	if citizen.nav_finished() or close or unreachable or _timeout <= 0.0:
